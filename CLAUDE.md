...
docker compose logs -f cron            # watch scheduler / brief output
...
- Markdown → HTML in emails goes through `marked` and `wrapHtml` in `cron/lib/email.js`. Don't add a different renderer.
- All three cron scripts follow the same shape: build prompt → `ask()` → `sendEmail()`. Match that shape when adding new jobs.
